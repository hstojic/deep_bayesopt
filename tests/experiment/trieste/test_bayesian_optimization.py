...
def test_bayesian_optimizer_with_mcdropout_finds_minima_of_scaled_branin(
    ...
    _test_optimizer_finds_minimum(
        MonteCarloDropout, num_steps, acquisition_rule, optimize_branin=True
    )
    ...
    if model_type is GaussianProcessRegression:
        ...
        if model_type is MCDropConnect:
            dropout_network = build_vanilla_keras_mcdropout(
                initial_data, rate=0.35, dropout_network=DropConnectNetwork
            )
        else:
            if isinstance(acquisition_rule, EfficientGlobalOptimization):
                ...
                rate = 0.1
            dropout_network = build_vanilla_keras_mcdropout(
                initial_data, rate=rate, dropout_network=DropoutNetwork
            )
