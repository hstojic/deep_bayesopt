...
def test_dropout_trajectory_sampler_returns_trajectory_function_with_correctly_shaped_output(
    ...
    model, _, _ = trieste_montecarlo_dropout_test(example_data, _RATE)
...
@pytest.mark.skip
def test_dropout_trajectory_sampler_returns_deterministic_trajectory(
    num_evals: int, batch_size: int, dim: int, num_outputs: int
) -> None:
    """
...
def test_dropout_trajectory_sampler_is_not_too_deterministic(seed: Optional[int]) -> None:
    """
    Different trajectories should have different internal state, even if we set the global RNG
    seed.
    """
    ...
    model1, _, _ = trieste_montecarlo_dropout_test(example_data, _RATE)
    model2, _, _ = trieste_montecarlo_dropout_test(example_data, _RATE)
    ...
    trajectory1 = DropoutTrajectorySampler(model1).get_trajectory()
    trajectory2 = DropoutTrajectorySampler(model2).get_trajectory()
    ...
    for _ in range(2):
        cast(dropout_trajectory, trajectory1).resample()
        cast(dropout_trajectory, trajectory2).resample()
        eval1 = trajectory1(test_data)
        eval2 = trajectory2(test_data)
...
def test_dropout_trajectory_sampler_samples_are_distinct_for_new_instances() -> None:
    ...
    def _get_trajectory_evaluation(
        model: ProbabilisticModel, seed: int
    ...
    eval_1 = _get_trajectory_evaluation(model, 0)(test_data)
    eval_2 = _get_trajectory_evaluation(model, 1)(test_data)
...
@pytest.mark.skip
@random_seed
def test_dropout_trajectory_sampler_samples_are_distinct_within_batch() -> None:
    """
...
@pytest.mark.skip
@random_seed
def test_dropout_trajectory_sampler_resample_with_new_sampler_does_not_change_old_sampler() -> None:
    """
...
@pytest.mark.skip
@random_seed
def test_dropout_trajectory_sampler_new_trajectories_diverge() -> None:
    """
...
@pytest.mark.skip
@random_seed
def test_dropout_trajectory_sampler_update_trajectory_updates_and_doesnt_retrace() -> None:
    """
    ...
    trajectory_sampler = DropoutTrajectorySampler(model)
...
@pytest.mark.skip
@random_seed
def test_dropout_trajectory_sampler_trajectory_on_subsets_same_as_set() -> None:
    """
    ...
    model, _, _ = trieste_montecarlo_dropout_test(train_data, _RATE)
...
@pytest.mark.skip
@random_seed
def test_dropout_trajectory_sampler_trajectory_is_continuous() -> None:
    """
...
@pytest.mark.skip
def test_ensemble_trajectory_sampler_returns_state(batch_size: int) -> None:
    dim = 3
    num_evals = 10
    ...
    sampler = DropoutTrajectorySampler(model)
    trajectory = cast(dropout_trajectory, sampler.get_trajectory())

    dtype = tf.int32
    rnd_state_name = "seeds"
