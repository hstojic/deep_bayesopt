...
class DeepEvidentialRegression(ApproximateBayesianModel):
    """
    This class builds the baseline Deep Evidential Regression model using Keras. The network
    architecture is a multilayer fully-connected feed-forward network.

    TODO -- cite Amini paper
    """
    ...
    def _gen_output_layer(self) -> tf.keras.Model:
        ...
        """
        The premise of this model is that it only works with univariate output data.
        Hence, we need to predict 4 parameters of the Normal-Inverse-Gamma distribution
        that defines the second-order distribution.
        """
        _flattened_output_shape = 4

        output_layer = tf.keras.Sequential(name="output_layer")
        output_layer.add(
            tf.keras.layers.Dense(
                units=_flattened_output_shape, dtype=self.input_tensor_spec.dtype
            )
        )
        return output_layer
    ...
    def predict_mean_and_var(
        ...
        Following cite Amini, we have the following analytic equations 
